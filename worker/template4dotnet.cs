...
{
	/*
	 *	Template field and section definitions
	 */
	...
	private void construct(string data, int oLast, int oEnd)
	{
		...
		while ((oCurr = data.IndexOf(DELIMITER, oLast)) != -1 && oCurr < oEnd)
		{
			...
			if ((oNext = data.IndexOf(DELIMITER, oCurr)) == -1 || oNext > oEnd)
				fail("bad syntax: missing delimiter");
				...
				&& String.Compare(SECTIONTAG_TAIL, 0, data, oNext, SECTIONTAG_TAIL_LEN) == 0)
			{
				...
				if (oNext == -1 || oNext > oEnd)
					fail ("Bad syntax: Missing section end tag");
				if (s.preceding != null)
					fail ("Bad syntax: Duplicate section");
				s.preceding = tail;
				s.tpl = new Template();
	...
	{
		if (tpl == this)
			fail ("appendSection: No section selected");
	...
	public string getSection(string key)
	{
		section s = tpl._getSection(key);
		if (s == null)
			fail("getSection: Cannot find section "+key);
	...
	{
		throw(new Exception("TemplateEngine.Template::"+msg));
	}
